...
class GuiFrame():
    ...
    def chooseLogs(self):
        ...
        self.beginParsingButton = w.getButton('Parse Selected Logs',self.parse,'activateParsing')
...
class Search():
    def __init__(self):
        ...
        logging.log('Time To Parse Query : %s', datetime.now() - starttime)
        self.showResultsList()
    def showResultsList(self):
        global frame
...
class Parse():
    ...
    def getLogs(self):
        ...
        for logFile, ParseChoice in logfiles.items():
            if ParseChoice:
                if logFile == '/var/log/exim_mainlog':
                    with open('/var/log/exim_mainlog', 'r') as f:
                        rawEximLogs.append(f.read())
                else:
                    with gzip.open(logFile, 'r') as f:
                        rawEximLogs.append(f.read())
        eximMainLog = " ".join(rawEximLogs)
        eximMainLog = eximMainLog.splitlines()
        return eximMainLog
    def parseIncoming(self,i):
        ...
        try:
            ...
            m = shlex.split('2019-01-01 00:00:00.000 [0000] xxxxxxxx <= notparsable@invalid.entry NA NA NA NA NA')
        else:
            m = shlex.split(i)
        if m[5] == '<>':
    ...
    def parseFailures(self, i):
        ...
        try:
            ...
            m = shlex.split('2019-01-01 00:00:00.000 [0000] xxxxxxxx CANNOT_PARSE NA NA NA NA NA')
        else:
            m = shlex.split(i)
    ...
    def parseOthers(self, i):
        ...
        try:
            ...
            entryList[pid][-1].msgdate = '2019-01-01'
        else:
            entryList[pid][-1].msgdate = m[0]
        try:
            m[1]
        except IndexError:
            entryList[pid][-1].msgtime = '00:00:00.000'
        else:
            entryList[pid][-1].msgtime = m[1]
        entryList[pid][-1].msgtime = m[1]
        entryList[pid][-1].msgpid = pid
        ...
        msgDateTime = entryList[pid][-1].msgdate + '_' + entryList[pid][-1].msgtime
        try:
            datetime.strptime(msgDateTime, s.datetimeformat)
        except ValueError:
            entryList[pid][-1].msgDateTime = datetime.strptime('2019-01-01_00:00:00.000', s.datetimeformat)
        else:    
            entryList[pid][-1].msgDateTime = datetime.strptime(msgDateTime, s.datetimeformat)
...
class Filters():
    ...
    def addRemoveFilters(self):
        ...
        frame.setFocus('body')
def newSearch(*args):
    logging.info('new search args = %s', args)
    global query
    global newQuery
    global newSearchSource
    #if newQuery[1]:
