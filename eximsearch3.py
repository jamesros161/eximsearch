...
class Search():
    def __init__(self):
        global results
        self.filterTypes = s.filterTypes
    ...
    def transposeLogFiles(self):
        ...
        for key, value in logfiles.items():
            if value:
                selectedLogs.append(key)
        return selectedLogs
    def transposeFilters(self):
        currentFilters = {}
        if type(queryFilter) != str:
            for filterType in self.filterTypes:
                if filterType != 'query':
                    ...
                    if query[1] == '':
                        ...
                        currentFilters[filterType] = [query[1]]
            for key in currentFilters:
                logging.log('Current Filters %s = %s', key, currentFilters[key])
    ...
    def filterLogs(self, filter):
        ...
        logging.log(":filterLogs :: Current Thread:: %s", threading.current_thread().getName())
        logging.info('filterLogs filter: %s', filter)
        ...
        if self.filterType == 'Date':
            ...
            for entry in newFilter:
                ...
                if not dateString:
                    ...
                    dateTimeObject = stringToDate(entry)
                    dateTimeFilter.append(datetime.strftime(dateTimeObject.date(), s.logDateFormat))
        if addFilterTrue:
